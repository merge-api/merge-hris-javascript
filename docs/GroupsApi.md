...
> PaginatedGroupList groupsList(x_account_token, opts)
...
let apiInstance = new MergeHrisApi.GroupsApi();
...
apiInstance.groupsList(x_account_token, opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
...
> Group groupsRetrieve(x_account_token, id, opts)
...
apiInstance.groupsRetrieve(x_account_token, id, opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
    console.log('API called successfully. Returned data: ' + data);
