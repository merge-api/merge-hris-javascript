...
> Benefit benefitsCreate(xAccountToken, opts)
...
let opts = {
  ...
  'createBenefit': new MergeHrisApi.CreateBenefit() // CreateBenefit | 
};
apiInstance.benefitsCreate(xAccountToken, opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
 ...
 **createBenefit** | [**CreateBenefit**](CreateBenefit.md)|  | [optional] 
